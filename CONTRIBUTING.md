...
- **More language pairs**:
  - Add optional mappings for other popular language pairs that have similar “wrong layout” problems (e.g., French–English, Russian–English), keeping the core flow the same.

- **Per-user presets**:
  - Allow saving a small list of mapping presets (e.g., “Arabic–English”, “Arabic–English + digits”), selectable from the tray menu without opening a complex settings window.

- **Lightweight usage statistics (local only)**:
  - Show a simple count in the tray tooltip or a small dialog: how many times the shortcut was used today/this session, with no external logging or analytics.

- **Optional smart-detection tweaks**:
  - Keep the same `ContainsArabic` approach, but add very small heuristics (like ignoring numbers/URLs) to avoid toggling when the selection is clearly not a sentence.

- **Simple ignore rules**:
  - Allow users to specify a short list of apps where LangFlip should not run (e.g., password managers), implemented as a basic process-name check in the correction flow.

- **Minimal UX touches**:
  - Add an optional, very small notification (balloon or short-lived popup) that confirms the conversion direction (e.g., “Converted: EN → AR”), which can be disabled in settings.
